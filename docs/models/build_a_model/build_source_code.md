...
To debug a model through the [linaro-forge](https://opus.nci.org.au/spaces/Help/pages/363659856/Linaro+Forge+HPC+Tools...) debugger, some minor changes to the build are required.
...
This should build executables compatible with the Linaro debugger. Note that if you have an existing build that was built without ```--keep-stage```, the model will need to be uninstalled and re-installed again.

### Running the model

To locate the specific executables generated by the build, use ```spack find --paths```, which will provide the locations of each of the components included in the current build. Use the relevant executables located in ```bin/<exe_name>``` at the location given by Spack. From this point, it should be possible to follow the [NCI instructions](https://opus.nci.org.au/spaces/Help/pages/363659856/Linaro+Forge+HPC+Tools...) on using Linaro Forge, substituting in the desired executable.
