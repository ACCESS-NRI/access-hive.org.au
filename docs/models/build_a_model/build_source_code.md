...
Debugging is an important part of the development process. Using a debugger can make this process significantly easier, allowing detailed inspection of the state of the code as it is running. The debugger used in this guide is Linaro DDT from [Linaro Forge](https://www.linaroforge.com/).

### Setting up the build

To debug a model through the linaro-forge debugger, some minor changes to the build are required.

1. Change the version of Openmpi to ```openmpi/4.1.3```.
2. Modify the compilation options to include debug information and prevent the compiler from re-ordering the code for the purpose of optimisation. This is done by appending the following to the model ```specs``` in the ```spack.yaml```:
...
This should build executables compatible with the Linaro debugger. Note that if you have an existing build that was built without ```--keep-stage```, the model will need to be uninstalled and re-installed again to retain the source code.

### Running the model through the debugger

The recommended way to run with a debugger is with ```payu```. To execute a debugging run via ```payu```, you must:

1. Locate the executables generated by the Spack build using ```spack find --paths```, which lists the installation directories for each package in the environment. The generated executables will be located at ```bin/<exe_name>``` in these locations.
2. Specify the paths to these executables in the ```exe``` section for each of the submodel involved in the desired configuration. For ESM1.5, these would be ```atmosphere```, ```ocean``` and ```ice```.
3. Tell ```payu``` to pipe the run through Linaro DDT by adding ```mpi: runcmd: ddt --connect mpirun```.

If you have set up the remote client for Linaro Forge via the [Connecting remotely](https://docs.linaroforge.com/25.0.2/html/forge/forge/connecting_to_a_remote_system/connecting_remotely.html) instructions, then you can simply ```payu run```, and the run should connect to your remote client. Alternatively, you can follow the [instructions from NCI](https://opus.nci.org.au/spaces/Help/pages/363659856/Linaro+Forge+HPC+Tools...) to run the debugger from an interactive job- in this instance, call ```payu-run``` (note the hyphen instead of a space) from the control directory while in the interactive job. It is recommended to significantly reduce the core count and simulation time to e.g. 8 cores and 1 day of sim time.

If you aren't running with ```payu```, then you can simply call the executables directly, making sure to prepend ```ddt --connect``` to ```mpirun```.
