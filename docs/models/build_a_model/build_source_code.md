...
!!! warning
    These steps assume you have already set up the _Linaro Forge_ remote client. If you haven't done this yet, follow the [Connecting remotely](https://docs.linaroforge.com/25.0.2/html/forge/forge/connecting_to_a_remote_system/connecting_remotely.html) instructions on their website. When filling out the remote launch settings for Gadi, set __Host Name__: `<username>@nci.org.au` and __Remote Installation Directory__:`/apps/linaro-forge/24.0.2`.

The recommended way to execute a debugging run is by using _payu_ as follows:

1. Locate the executables generated by the _Spack_ build by running:
    ```
    spack find --paths
    ```
