...
If you have set up the remote client for Linaro Forge via the [Connecting remotely](https://docs.linaroforge.com/25.0.2/html/forge/forge/connecting_to_a_remote_system/connecting_remotely.html) instructions, then you can simply ```payu run```, and the run should connect to your remote client. Alternatively, you can follow the [instructions from NCI](https://opus.nci.org.au/spaces/Help/pages/363659856/Linaro+Forge+HPC+Tools...) to run the debugger from an interactive job- in this instance, call ```payu-run``` (note the hyphen instead of a space) from the control directory while in the interactive job.
